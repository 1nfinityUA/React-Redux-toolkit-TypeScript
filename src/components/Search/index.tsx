...
// для пошуку по інпуту на сайті
const Search: React.FC = () => {
    const dispatch = useDispatch();
    // створили локально значення input для бистрого відображення на екрані
    // а глобально воно буде мінятись через 1000мс
    const [value, setValue] = React.useState("");
    const inputRef = React.useRef<HTMLInputElement>(null);
    const onClickClear = () => {
        dispatch(setSearchValue(''));
        setValue("");
        // знак питання виконує функцію якшо є курент то йдемо далі і робимо фокус
        inputRef.current?.focus();
    };
    // для того щоб зробити затримку перед змінюванням інпуту від вводу тексту (щоб не робити на кожну букву запит)
    ...
    return (
        <div className={styles.root}>
            ...
            <input
                // valaue для того зоб інтуп залежав від змінної бо коли немає залежності тоді не буде стирати
                ref={inputRef}
                value={value}
            ...
            />
            {/*якщо пошук пустий то не відображати хрестик */}
            {value && (
                <img
